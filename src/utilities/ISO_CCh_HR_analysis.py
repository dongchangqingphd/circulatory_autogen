...
def compute_peak_period(timeline,V_data):
    ...
    if len(peak_times_max) > 1:
        ...
        avg_period = np.nan  # cannot find enough peak
...
#file_path = "/home/cdon822/Documents/OpenCOR/NESAN-calibrated2.cellml"
file_path = "/home/cdon822/Documents/GIT_files/circulatory_autogen/generated_models/SANHR13/SANHR13.cellml"
...
#obs_list = ['Rate_modulation_experiments/ISO_init','Membrane/V']
obs_list = ['NES/ISO_init','hrv/V']
...
#reset_param_names2 = ['Rate_modulation_experiments/ACh_init','Rate_modulation_experiments/ISO_init']
reset_param_names2 = ['NES/ACh_init','NES/ISO_init']
#set initial value
reset_param_val2 = [0.11,1.0]
simulation_time = 10
simulation_dt = 0.0001
max_time = simulation_time + simulation_dt
#define a variable that save result from iteration
inputs2 = np.arange(0,max_time,simulation_dt)
...
for ii in range(1,51):
    ...
    print("ii=",ii)
    x = SimulationHelper(file_path, simulation_dt, simulation_time, pre_time=0)
    reset_param_val2[0] = reset_param_val2[0]*1.2
    x.set_param_vals(reset_param_names2, reset_param_val2)
    x.run()
    z = x.get_results(obs_list)
    z1 = z[1][0]
    #z_out = z[1][100]
    ...
    outputs = z1.copy()
    print("ACh=",reset_param_val2[0])
    frequency_HR = compute_peak_period(inputs2,outputs)
    NEsC.append(reset_param_val2[0])
    HRfC.append(frequency_HR)
...
x_rst.set_param_vals(reset_param_names, reset_param_val)
x_rst.run()
...
print("HRfC=",HRfC)
HRfC2 = [x2*100.0/frequency_HR_std for x2 in HRfC]

#set the final curve saved address and image name
save_addr1 = Outputfile_address + "/"+save_newfilename1+"/ISO_HR-test.png"
save_addr2 = Outputfile_address + "/"+save_newfilename1+"/CCh_HR-test.png"


plt.plot(NEs,HRf2,marker='.',linestyle='-',color='b',label='calibrated')
plt.title(str_title)
plt.xscale('log')
...
x1 = [0.1,0.5,1,10,100,1000]
y1 = [75.9,90.62,100,107.28,113.82,115.29]
yerr = [9.576,8.855,9.332,7.868,6.615,3.23]
...
plt.xlabel('ISO concentration [nM]')
...
plt.xscale('log')
plt.xticks([0.1, 1, 10, 100, 1000], labels=["0.1", "1", "10", "100", "1000"])
plt.plot(NEsC,HRfC2,marker='.',linestyle='-',color='b',label='calibrated')
#add error bar in curve
x2 = [100,500,1000]
y2 = [73,45,29]
yerr = [10.5,15,11]
